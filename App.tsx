...
export default function App() {
  ...
  // On mount: check for existing users
  useEffect(() => {
    const recentUsers = StorageService.getRecentUsers();
    if (recentUsers.length === 0) {
      // Brand new user - go directly to landing page to upload/capture
      setView(AppView.LANDING);
    } else {
      // Existing users - show login screen for quick access
  ...
  const handleLogin = (
    ...
    if (type === "PARTNER") {
      ...
      const isValid =
        (code && code.length > 2) || loggedInUser.role === "PARTNER";
  ...
  const handleUnlock360 = async () => {
    ...
    try {
      ...
      const remainingViews = await generateRemainingViews(
        originalImage,
        prompt,
      );
  ...
  const handleUndo = () => {
    if (currentStackIndex > 0) {
      ...
      setGenerationState((prev) => ({
        ...prev,
        resultImages: historyStack[prevIndex],
        unlockedAngles: false,
      }));
    }
  };
  ...
  const handleRedo = () => {
    if (currentStackIndex < historyStack.length - 1) {
      ...
      setGenerationState((prev) => ({
        ...prev,
        resultImages: historyStack[nextIndex],
      }));
    }
  };
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-brand-100 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          >
            ...
            <span className="font-serif text-2xl font-bold tracking-tight text-brand-900 hidden sm:block">
              Prelook Studio
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
            ...
            <div
              ...
              className="cursor-pointer bg-brand-100 hover:bg-brand-200 px-3 py-1 rounded-full flex items-center gap-2 transition-colors"
            >
              <div className="bg-brand-900 text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold">
                {user.credits}
              </div>
              <span className="text-xs font-bold uppercase text-brand-800 tracking-wider hidden sm:inline">
                Credits
              </span>
            </div>
            ...
            <div
              ...
              className="hidden md:flex items-center gap-3 cursor-pointer hover:opacity-70 border-l border-brand-200 pl-4"
            >
              <div className="text-right">
                <p className="text-xs font-bold uppercase tracking-widest text-brand-900">
                  {user.name}
                </p>
              ...
              <div className="w-9 h-9 rounded-full border border-brand-200 p-0.5">
                <img
                  src={user.avatar}
                  alt="Avatar"
                  className="w-full h-full rounded-full bg-brand-100"
                />
              </div>
            </div>
            ...
            <button
              ...
              className="p-2 hover:bg-brand-100 rounded-full text-brand-400 hover:text-brand-900 transition-colors"
            >
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-700 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-white to-brand-50">
            <div className="max-w-4xl mx-auto space-y-8">
              ...
              <h1 className="font-serif text-6xl md:text-7xl lg:text-8xl text-brand-900 leading-[0.95]">
                Discover Your
                <br />
                <span className="text-brand-500 italic font-light">
                  Perfect Look
                </span>
              </h1>
            ...
            {history.length > 0 && (
              <div className="mt-12 w-full max-w-7xl px-4">
                <h3 className="text-left font-serif text-xl mb-4 text-brand-800">
                  Your Recent Styles
                </h3>
                <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                  {history.slice(0, 6).map((session) => (
                    <div
                      ...
                      className="cursor-pointer group relative aspect-[3/4] rounded-xl overflow-hidden"
                    >
                      <img
                        src={
                          session.resultImages.front || session.originalImage
                        }
            ...
            <div className="flex flex-1 overflow-hidden">
              <div className="flex-1 relative md:mr-20">
                ...
                {!generationState.isLoading &&
                  generationState.resultImages.front && (
                    <div
                      id="salon-finder"
                      className="bg-brand-50 border-t border-brand-200 pb-32"
                    >
