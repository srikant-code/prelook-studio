...
export const ResultView: React.FC<ResultViewProps> = ({
  ...
  const ViewCard = ({
    ...
    <div
      className={`relative group rounded-3xl overflow-hidden aspect-[3/4] transition-all duration-500 
    ...
    >
      ...
      {isLocked && !isLoading && (
        <div className="absolute inset-0 flex flex-col items-center justify-center z-20 bg-black/10 transition-colors hover:bg-black/20">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest text-brand-900 bg-white/90 px-3 py-1 rounded-full">
            Unlock View
          </span>
        </div>
      )}
      ...
      <div className="absolute bottom-4 left-4 text-white z-10 pointer-events-none">
        <span className="text-xs font-medium tracking-wide opacity-90 drop-shadow-md">
          {title}
        </span>
      </div>
    </div>
      ...
      <div className="h-full w-full overflow-y-auto p-4 pb-48 md:pb-64 md:p-8 bento-scroll">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            <div>
              <h2 className="text-2xl font-serif text-brand-800">
                Your Transformation
              </h2>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-serif text-lg mb-1">
                    Make this {currentStyleName} a reality
                  </h3>
                  <p className="text-sm text-brand-200 opacity-90">
                    <span className="font-bold text-white">Top Stylists</span>{" "}
                    in Bhubaneswar can create this look for you.
                  </p>
                </div>
              </div>
              <Button
                onClick={onBookNow}
                className="bg-white text-brand-900 hover:bg-brand-50 border-none min-w-[140px]"
              >
                Book Appointment
              </Button>
            </div>
