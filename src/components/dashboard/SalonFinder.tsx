...
export const SalonFinder: React.FC<SalonFinderProps> = ({
  onBookingComplete,
  userEmail,
}) => {
  ...
  const confirmBooking = () => {
    if (
      ...
      console.log(
        `Sending booking confirmation to manish.rath5240@gmail.com for booking at ${selectedSalon.title}`,
      );
    ...
    return (
      <div className="border-t border-brand-200 bg-brand-50 p-12 text-center animate-fade-in">
        ...
        <p className="text-brand-600 mb-4 max-w-md mx-auto">
          You are booked at{" "}
          <span className="font-bold">{selectedSalon?.title}</span> with{" "}
          <span className="font-bold">
            {selectedStylist || "Next Available Stylist"}
          </span>
          .
        </p>
        <div className="bg-blue-50 text-blue-800 p-4 rounded-xl max-w-md mx-auto text-sm mb-8">
          An email confirmation has been sent to{" "}
          <span className="font-bold underline">manish.rath5240@gmail.com</span>
        </div>
        <div className="flex justify-center">
          <Button onClick={() => setStep("LIST")} variant="outline">
            Browse More Salons
          </Button>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <button
              ...
              className="flex items-center gap-2 text-brand-500 hover:text-brand-900 text-sm font-bold uppercase tracking-widest"
            >
              <ArrowLeft className="w-4 h-4" /> Back to Store
            </button>

            <div>
              <h2 className="font-serif text-3xl text-brand-900 mb-2">
                Checkout
              </h2>
              <p className="text-brand-500">
                Complete your booking details for {selectedSalon.title}.
              </p>
            </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-brand-100 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-bold text-brand-400 mb-3">
                    Date
                  </label>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {["10:00 AM", "1:00 PM", "3:30 PM", "5:00 PM"].map(
                      (time) => (
                        <button
                          key={time}
            ...
            <div className="bg-white p-6 rounded-3xl border border-brand-100 shadow-sm">
              ...
              <div className="flex items-center gap-4 p-4 border border-brand-200 rounded-xl bg-brand-50/50">
                ...
                <button className="text-xs font-bold text-brand-900 underline">
                  Change
                </button>
              </div>
            </div>
              ...
              <div className="space-y-4 border-b border-white/20 pb-6 mb-6">
                ...
                <div className="flex justify-between items-start">
                  ...
                  <p className="font-bold text-sm text-right">
                    {selectedStylist || "Any Expert"}
                  </p>
                </div>
              </div>

              <div className="flex justify-between items-end mb-8">
                <p className="text-sm text-brand-300 uppercase tracking-widest">
                  Total
                </p>
      ...
      <div className="border-t border-brand-200 bg-brand-50 animate-fade-in min-h-[600px] mb-24">
        ...
        <div className="h-64 md:h-80 w-full relative overflow-hidden">
          <img
            ...
            alt={selectedSalon.title}
          />
          <div className="absolute inset-0 bg-gradient-to-t from-brand-900/80 to-transparent"></div>
          <div className="absolute top-6 left-6 md:left-12">
            <button
              onClick={() => setStep("LIST")}
              className="flex items-center gap-2 text-white/90 hover:text-white text-sm font-bold uppercase tracking-widest bg-black/20 backdrop-blur-md px-4 py-2 rounded-full hover:bg-black/40 transition-all"
            >
          ...
          <div className="absolute bottom-6 md:bottom-12 left-6 md:left-12 text-white max-w-2xl">
            <div className="flex items-center gap-2 mb-2">
              <span className="px-2 py-0.5 bg-white/20 backdrop-blur-md rounded text-[10px] font-bold uppercase tracking-widest">
                {selectedSalon.specialty}
              </span>
            ...
            <p className="text-brand-100 flex items-center gap-2 text-sm">
              <MapPin className="w-4 h-4" /> {selectedSalon.address}
            </p>
          </div>
        </div>
            ...
            <div>
              ...
              <div className="space-y-2 text-sm text-brand-600">
                <div className="flex justify-between">
                  <span>Mon - Fri</span> <span>10:00 AM - 9:00 PM</span>
                </div>
                <div className="flex justify-between">
                  <span>Saturday</span> <span>10:00 AM - 9:00 PM</span>
                </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="grid grid-cols-1 gap-4">
              {selectedSalon.services?.map((service) => (
                <div
                  key={service.id}
                  className="group flex items-center justify-between p-6 bg-white rounded-2xl border border-brand-100 hover:border-brand-300 hover:shadow-lg transition-all cursor-pointer"
                  onClick={() => handleBookService(service)}
                >
                  <div>
                    <h4 className="font-serif text-lg text-brand-900 font-bold group-hover:text-brand-700 transition-colors">
                      {service.name}
                    </h4>
    ...
    <div className="border-t border-brand-200 bg-brand-50 p-8 min-h-[600px] mb-24">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <h3 className="font-serif text-3xl text-brand-900 mb-3">
            Partner Salons in Bhubaneswar
          </h3>
          <p className="text-brand-500 max-w-lg mx-auto">
            Book appointments with top-rated luxury salons including Looks, Toni
            & Guy, and Jawed Habib.
          </p>
        </div>
        ...
        {salons.length === 0 && !loading && (
          <div className="text-center py-12">
            <Button
              ...
              variant="primary"
              className="min-w-[200px] shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all"
            >
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <span className="text-brand-500 text-sm font-bold uppercase tracking-widest">
              Locating salons in Bhubaneswar...
            </span>
          </div>
        )}
        ...
        {error && (
          <div className="text-center py-8">
            <div className="text-red-500 text-sm mb-4 bg-red-50 inline-block px-4 py-2 rounded-full">
              {error}
            </div>
            <div>
              <Button onClick={handleFindSalons} variant="outline">
                Try Again
              </Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-slide-up pb-12">
            {salons.map((salon) => (
              <div
                ...
                className="group bg-white rounded-3xl overflow-hidden border border-brand-100 hover:border-brand-300 hover:shadow-xl transition-all cursor-pointer flex flex-col h-full"
              >
                {/* Card Image */}
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <div className="mb-4">
                    <h4 className="font-serif font-bold text-brand-900 text-xl line-clamp-1 mb-1 group-hover:text-brand-600 transition-colors">
                      {salon.title}
                    </h4>
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    <div className="flex items-center justify-between text-xs border-b border-brand-50 pb-2">
                      ...
                      <span className="font-bold text-green-600">
                        {salon.availableSlots} slots today
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-auto pt-4 flex gap-3">
                    ...
                    <a
                      ...
                      className="p-3 border border-brand-200 rounded-xl hover:bg-brand-50 text-brand-400 hover:text-brand-900 transition-colors"
                    >
