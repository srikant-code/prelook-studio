    ...
    <div className="flex-1 flex flex-col bg-brand-50 overflow-hidden">
      <div className="bg-white border-b border-brand-200 p-8">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="font-serif text-3xl text-brand-900 mb-1">
              Hello, {user.name}
            </h1>
          ...
          <button
            onClick={onClose}
            className="text-sm font-bold uppercase tracking-wider text-brand-400 hover:text-brand-900"
          >
            Back to Studio
          </button>
        </div>
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white p-6 rounded-3xl border border-brand-100 shadow-sm">
              ...
              <div className="text-5xl font-serif text-brand-900 mb-2">
                {user.credits}
              </div>
              <p className="text-sm text-brand-500 mb-6">Available Credits</p>
              <button
                onClick={onClose}
                className="w-full py-3 bg-brand-900 text-white rounded-xl text-xs font-bold uppercase tracking-widest"
              >
                Top Up Credits
              </button>
            </div>
              ...
              {bookings.length > 0 ? (
                <div className="space-y-4">
                  {bookings.map((booking) => (
                    <div
                      key={booking.id}
                      className="flex items-center justify-between p-4 bg-brand-50 rounded-2xl border border-brand-100"
                    >
                      <div>
