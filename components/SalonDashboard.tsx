...
import { Calendar, Wallet, Users, TrendingUp, Scissors, Search, CheckCircle, XCircle, Clock } from 'lucide-react';
...
export const SalonDashboard: React.FC<SalonDashboardProps> = ({ user, salonCode, onLogout }) => {
  
  // Mock Data for the dashboard
  const stats = [
    { label: 'Weekly Bookings', value: '24', icon: Calendar, color: 'text-brand-900' },
    { label: 'Revenue (Est.)', value: '$1,840', icon: TrendingUp, color: 'text-green-600' },
    { label: 'AI Sessions Used', value: '45', icon: Scissors, color: 'text-brand-600' },
    { label: 'Salon Credits', value: '120', icon: Wallet, color: 'text-accent' },
  ];

  const recentBookings: Booking[] = [
    { id: '1', salonName: 'Studio', service: 'Balayage & Cut', date: 'Today', time: '2:00 PM', price: '$120', status: 'CONFIRMED', timestamp: 123 },
    { id: '2', salonName: 'Studio', service: 'Mens Fade', date: 'Today', time: '3:30 PM', price: '$45', status: 'CONFIRMED', timestamp: 124 },
    { id: '3', salonName: 'Studio', service: 'Consultation', date: 'Tomorrow', time: '10:00 AM', price: '$0', status: 'CONFIRMED', timestamp: 125 },
    { id: '4', salonName: 'Studio', service: 'Full Color', date: 'Tomorrow', time: '11:30 AM', price: '$150', status: 'CANCELLED', timestamp: 126 },
  ];
        ...
        <div className="bg-white border-b border-brand-200 p-6 sticky top-0 z-30">
            <div className="max-w-7xl mx-auto flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="font-serif text-xl text-brand-900 font-bold">Luxe Salon Admin</h1>
                        <p className="text-xs text-brand-500 uppercase tracking-wider">ID: {salonCode}</p>
                    </div>
                </div>
                <div className="flex items-center gap-4">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8">
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {stats.map((stat, idx) => (
                        <div key={idx} className="bg-white p-5 rounded-2xl border border-brand-100 shadow-sm flex flex-col justify-between h-32 hover:shadow-md transition-shadow">
                            <div className="flex justify-between items-start">
                                <span className="text-xs font-bold uppercase text-brand-400 tracking-widest">{stat.label}</span>
                                <stat.icon className={`w-5 h-5 ${stat.color}`} />
                            </div>
                            <span className="text-3xl font-serif text-brand-900 font-medium">{stat.value}</span>
                        </div>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 bg-white rounded-3xl border border-brand-100 shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-brand-100 flex items-center justify-between">
                            <h3 className="font-serif text-xl text-brand-900">Incoming Appointments</h3>
                                ...
                                <tbody className="divide-y divide-brand-100">
                                    {recentBookings.map((booking) => (
                                        <tr key={booking.id} className="hover:bg-brand-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-brand-600">{booking.service}</td>
                                            ...
                                            <td className="px-6 py-4">
                                                <button className="text-brand-400 hover:text-brand-900 text-xs underline font-bold">Details</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                    ...
                    <div className="space-y-6">
                        <div className="bg-brand-900 text-white rounded-3xl p-6 shadow-xl relative overflow-hidden">
                             ...
                             <h3 className="font-serif text-2xl mb-2 relative z-10">In-Salon AI Mode</h3>
                             <p className="text-brand-300 text-sm mb-6 relative z-10 max-w-[80%]">
                                 Use salon credits to generate styles for walk-in clients without them needing an account.
                             </p>
                        ...
                        <div className="bg-white rounded-3xl border border-brand-100 p-6 shadow-sm">
                            <h3 className="font-bold text-brand-900 uppercase tracking-widest text-xs mb-4">Verification Requests</h3>
                            <div className="space-y-4">
                                <div className="flex items-center justify-between p-3 bg-brand-50 rounded-xl border border-brand-100">
                                    <div>
                                        <p className="font-bold text-brand-900 text-sm">Booking #B-921</p>
                                        <p className="text-xs text-brand-500">Checking in now</p>
                                    </div>
                                    <button className="px-3 py-1.5 bg-brand-900 text-white rounded-lg text-xs font-bold">Verify</button>
                                </div>
                                <p className="text-center text-xs text-brand-400 mt-2">No other pending requests</p>
                            </div>
                        </div>
                    </div>
